...
function extractMetaChainResult(
  ...
  startLineIdx: number,
  tagPrefix: string
): { callTitle: string; azureId: number | undefined; tags: string[]; titleLineIdx: number } | null {
  let azureId: number | undefined;
  const tags: string[] = [];
  ...
  for (let i = startLineIdx; i < scanEnd; i++) {
    ...
    const kvRe = new RegExp(`\\.meta\\s*\\(\\s*['"]${tagPrefix}['"]\\s*,\\s*['"]([\\d]+)['"]`);
    ...
    if (azureId === undefined) {
      const objRe = new RegExp(`\\.meta\\s*\\(\\s*\\{[^}]*['"]?${tagPrefix}['"]?\\s*:\\s*['"]([\\d]+)['"]`);
