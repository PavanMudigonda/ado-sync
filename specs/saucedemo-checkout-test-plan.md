...
Expected results:
- User is redirected to `/inventory.html`. Product list is visible.
...
9. The item appears with the same name and price.
10. Item total equals the sum of item prices.
11. Tax is displayed and > 0.
12. Total equals item total + tax.
13. Click `Finish`.

Expected results:
- After Finish, user sees the completion page with text like "THANK YOU FOR YOUR ORDER". Cart badge is cleared (often back to zero) after completion or on returning home.
